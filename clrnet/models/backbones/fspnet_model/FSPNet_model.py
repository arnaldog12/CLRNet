...
class Converter(nn.Module):
    def __init__(self, dim_in, dim_temp=384, img_size=384, mids=4):
        ...
        self.conv_f1_extend = nn.Conv2d(
            self.dim_temp, self.dim_in, kernel_size=1, bias=False
        )
    ...
    def forward(self, token_pair):
        ...
        for index in range(len(token_pair) // 2):
            ...
            f1_ = (
                f1_.permute(0, 2, 1)
                .view(bs, chs, int(self.img_size // 16), int(self.img_size // 16))
                .contiguous()
            )
            # [8,768,24,24]
            f2_ = (
            ...
            fc_att = torch.nn.functional.softmax(fc, dim=1)[:, 1, :, :].unsqueeze(
                1
            )  # [8,1,24,24]

            # f1 pass
            f1_Q = (
                self.conv_f1_Q(f1).view(bs, self.dim_temp, -1).contiguous()
            )  # [8,384,576] [bs,chs,24*24]
            f1_K = self.conv_f1_K(f1)  # [8,384,24,24]
            f1_masked = f1_K * fc_att  # [8,384,24,24]
            f1_V = self.ap_f1(f1_masked)[:, :, 1:-1, 1:-1].reshape(
                bs, self.dim_temp, -1
            )  # [8,384,16]

            f1_proj_reshaped = torch.matmul(
                f1_V.permute(0, 2, 1), f1_K.reshape(bs, self.dim_temp, -1)
            )  # [8,16,576]
            f1_proj_reshaped = torch.nn.functional.softmax(
                f1_proj_reshaped, dim=1
            )  # [8,16,576] Tv

            f1_rproj_reshaped = f1_proj_reshaped  # [8,16,576]
            f1_n_state = torch.matmul(
            ...
            f1_state = f1_state_reshaped.view(
                bs, self.dim_temp, *f1.size()[2:]
            )  # [16,384,24,24]
            f1_out = f1_ + (self.conv_f1_extend(f1_state))  # [16,768,24,24]

            # f2 pass
            f2_Q = (
                self.conv_f2_Q(f2).view(bs, self.dim_temp, -1).contiguous()
            )  # [8,384,576] [bs,chs,24*24]
            f2_K = self.conv_f2_K(f2)  # [8,384,24,24]
            f2_masked = f2_K * fc_att  # [8,384,24,24]
            f2_V = self.ap_f2(f2_masked)[:, :, 1:-1, 1:-1].reshape(
                bs, self.dim_temp, -1
            )  # [8,384,16]

            f2_proj_reshaped = torch.matmul(
                f2_V.permute(0, 2, 1), f2_K.reshape(bs, self.dim_temp, -1)
            )  # [8,16,576]
            f2_proj_reshaped = torch.nn.functional.softmax(
                f2_proj_reshaped, dim=1
            )  # [8,16,576]

            f2_rproj_reshaped = f2_proj_reshaped  # [8,16,576]
            f2_n_state = torch.matmul(
            ...
            f2_state = f2_state_reshaped.view(
                bs, self.dim_temp, *f2.size()[2:]
            )  # [16,384,24,24]
...
class GroupFusion(nn.Module):
    def __init__(self, in_chs, out_chs, start=False):  # 768, 384
        ...
        self.gf2 = nn.Sequential(
            nn.Conv2d((temp_chs + temp_chs), temp_chs, 1, bias=False),
            nn.BatchNorm2d(temp_chs),
            nn.ReLU(inplace=True),
            nn.Conv2d(temp_chs, temp_chs, 3, padding=1, bias=False),
            nn.BatchNorm2d(temp_chs),
...
class FSPNet(nn.Module):
    ...
    def group_converter_fn(self, tokens):
        group_converter_ls = [
            self.group_converter_0,
            self.group_converter_1,
            self.group_converter_2,
            self.group_converter_3,
            self.group_converter_4,
            self.group_converter_5,
        ]
        tokens_ls = []
    ...
    def group_pyramid_decode(self, feature):
        ...
        f1_1_o, f2_1 = self.gf1_1(
            torch.cat((feature[-11], f1_2_l), dim=1), feature[-12]
        )  # f1_1_l [bs,768,24,24]
        # layer2 out
        f2_3_l, f3_3 = self.gf2_3(f2_6, f2_5)
        f2_2_l, f3_2 = self.gf2_2(torch.cat((f2_4, f2_3_l), dim=1), f2_3)
        f2_1_o, f3_1 = self.gf2_1(
            torch.cat((f2_2, f2_2_l), dim=1), f2_1
        )  # f2_1_l [bs,384,48,48]
        # layer3 out
        f3_2_l, f4_2 = self.gf3_2(f3_3, f3_2)
        f3_1_o, f4_1 = self.gf3_1(
            torch.cat((f3_2, f3_2_l), dim=1), f3_1
        )  # f3_1_l [bs,192,96,96]
        # layer4 out
    ...
    def pred_out(self, gpd_outs):
        return (
            self.out1(gpd_outs[0]),
            self.out2(gpd_outs[1]),
            self.out3(gpd_outs[2]),
            self.out4(gpd_outs[3]),
        )
